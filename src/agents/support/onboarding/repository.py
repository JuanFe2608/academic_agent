...
class RepositoryConfigurationError(OnboardingRepositoryError):
    """La capa de persistencia no esta configurada correctamente."""
...
class InMemoryOnboardingRepository:
    ...
    def create_student(self, profile: Any) -> int:
        institutional_email = str(profile_value(profile, "institutional_email") or "")
        student_code = str(profile_value(profile, "student_code") or "")
        ...
        payload = {
            "id": student_id,
            "full_name": profile_value(profile, "full_name"),
            "student_code": student_code,
            "age": profile_value(profile, "age"),
            "institutional_email": institutional_email,
            "email_verified": bool(profile_value(profile, "email_verified", False)),
            "academic_program": profile_value(profile, "academic_program"),
            "supported_program": profile_value(profile, "supported_program"),
            "semester": profile_value(profile, "semester"),
            "average_grade": profile_value(profile, "average_grade"),
...
class PostgresOnboardingRepository:
    ...
    def create_student(self, profile: Any) -> int:
        ...
        academic_program = profile_value(profile, "academic_program")
        supported_program = bool(profile_value(profile, "supported_program", False))
        ...
        with self._connect() as conn:
            ...
            row = conn.execute(
                ...
                (
                    ...
                    student_code,
                    profile_value(profile, "age"),
                    institutional_email,
                    bool(profile_value(profile, "email_verified", False)),
                    program_id,
                    supported_program,
                    profile_value(profile, "semester"),
                    profile_value(profile, "average_grade"),
                ),
            ).fetchone()
            conn.commit()
...
def build_postgres_repository(database_url: str | None) -> PostgresOnboardingRepository:
    ...
    if not database_url:
        raise RepositoryConfigurationError(
            "ACADEMIC_AGENT_DATABASE_URL no esta configurada."
        )
    return PostgresOnboardingRepository(database_url)
...
def _load_psycopg() -> tuple[Any, Any]:
    try:
        ...
        raise RepositoryConfigurationError(
            "psycopg no esta instalado. Agrega la dependencia para usar PostgreSQL."
        ) from exc
